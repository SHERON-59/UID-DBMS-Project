...
const CBSEManagementSystem = () => {
  ...
  const fetchData = async (endpoint) => {
    try {
      ...
      console.log(`Fetched ${endpoint}:`, result); // Debug log
  ...
  const openModal = (type, data = {}) => {
    ...
    setFormData({
      ...
      examDate: data.exam_date ? new Date(data.exam_date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
      ...
      studentRollNumber: data.student_roll_number || '',
      studentName: data.student_name || '',
      classStandard: data.class_standard ? data.class_standard.toString() : '10',
      answerBookId: data.answer_book_id || '',
      marksAssigned: data.marks_assigned ? data.marks_assigned.toString() : '',
      evaluationDate: data.evaluation_date || '',
      remarks: data.remarks || '',
      examinerName: data.examiner_name || '',
      qualification: data.qualification || '',
      experienceYears: data.experience_years ? data.experience_years.toString() : '',
    });
    setShowModal(true);
  };
  ...
  const renderModal = () => {
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  onClick={() => handleSubmit('students', {
                    student_roll_number: formData.studentRollNumber,
    ...
    if (modalType === 'answerSheet') {
      return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-lg">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">Add Answer Sheet</h2>
              <button
                onClick={() => setShowModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Answer Book ID</label>
                <input
                  type="text"
                  name="answerBookId"
                  value={formData.answerBookId}
                  onChange={handleInputChange}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  required
                />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Marks Assigned</label>
                <input
                  type="number"
                  name="marksAssigned"
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  onClick={() => handleSubmit('answer-sheets', {
                    answer_book_id: formData.answerBookId,
                    student_roll_number: formData.studentRollNumber,
                    subject_id: formData.subjectId,
                    examiner_id: formData.examinerId,
                    marks_assigned: formData.marksAssigned,
    ...
    if (modalType === 'examiner') {
      return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-lg">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">Add Examiner</h2>
              <button
                onClick={() => setShowModal(false)}
                className="text-gray-500 hover:text-gray-700"
              >
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  onClick={() => handleSubmit('examiners', {
